...
fn walk(field: Field, start: Position, height: Int, width: Int) -> Int {
  let reachable_targets = walk_rec(field, height, width, start, 0)
...
fn walk_rec(
  ...
  let #(i, j) = position
  use <- bool.guard(i < 0 || i >= height || j < 0 || j >= width, set.new())

  let level = field |> get_level(position)
  use <- bool.guard(level != reachable_level, set.new())

  use <- bool.guard(level == 9, set.new() |> set.insert(position))

  [#(i + 1, j), #(i, j + 1), #(i - 1, j), #(i, j - 1)]
  |> list.map(fn(position) {
    walk_rec(field, height, width, position, level + 1)
  })
  |> list.fold(set.new(), fn(acc, result) {
    acc
    |> set.union(result)
  })
}
